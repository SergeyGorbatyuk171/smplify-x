...
class Ui_CameraWidget(QtWidgets.QMainWindow, Ui_CameraWidget_Base):
    ...
    def __init__(self, camera, frustum, draw, camera_params):
        ...
        self.draw = draw
        self.camera_params = deepcopy(camera_params)
    ...
    def _reset(self):
        ...
        focal_len = self.camera_params['f'][0] / float(self.frustum['width'])
        normalized_center = self.camera_params['c'] / np.array([self.frustum['width'], self.frustum['height']])
        self.set_values(self.camera_params['t'], self.camera_params['rt'], focal_len,
                        normalized_center, self.camera_params['k'])
